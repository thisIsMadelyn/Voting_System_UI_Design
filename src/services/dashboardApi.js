...
export async function fetchEvents() {
    ...
    // data is now List<Events> — flat array
    return data.map(e => ({
        id:        e.id ?? e.Id,          // entity has capital Id — cover both
...
//         startTime: e.startTime,
//         title:     e.title,
//         hostingLC: e.hostingLC ?? e.location ?? 'TBD',
//         type:      e.type ?? 'OPERATIONAL',
//     }))
// }
...
export async function fetchPolls() {
    const res = await fetch(`${BASE}/polls`, { headers: authHeaders() })
...
export async function searchByDiscordTag(tag) {
    const res = await fetch(`${BASE}/users/search/discord?tag=${encodeURIComponent(tag)}`, { headers: authHeaders() })
