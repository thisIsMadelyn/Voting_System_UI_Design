...
import { usePolls, useCreatePoll } from '../services/usePolls'
import useAuthStore from '../services/AuthStore'
import { useHasVoted, useVoteCount, useCastVote, useElectionResults } from '../services/useVoting'
import { useCheckIn } from '../services/useAttendance'
import { useDismissed } from '../services/useDismissed'
        ...
        <div className={styles.card}>
            ...
            {poll.isActive && (
                <button
                    ...
                    disabled={hasVoted}
                >
                    {hasVoted ? 'Already Voted ✓' : expanded ? 'Hide Voting' : 'Cast Vote'}
                </button>
            )}

            {expanded && !hasVoted && poll.options?.length > 0 && (
                        ...
                        <div className={styles.results}>
                            ...
                            {results.candidates?.map((c, i) => (
                                <div key={i} className={styles.resultRow}>
                                    ...
                                    <span className={styles.resultVotes}>{c.voteCount} votes</span>
                                </div>
                            ))}
                            {results.winner && (
...
export default function PollsPage() {
    ...
    const handleSubmit = (e) => {
        ...
        createPoll({
            ...
            onSuccess: () => {
                setForm({ title: '', description: '', meetingId: '', majorityType: 'ABSOLUTE', electoralBodyCount: '', candidateNames: [''] })
                setShowForm(false)
            }
        })
    }
                    ...
                    <form onSubmit={handleSubmit} className={styles.form}>
                        <div className={styles.row}>
                            <div className={styles.field}>
                                ...
                                <input className={styles.input} name="title" placeholder="Poll title" value={form.title} onChange={handleChange} required />
                            </div>
                            <div className={styles.field}>
                                <label className={styles.label}>Meeting ID</label>
                                <input className={styles.input} name="meetingId" type="number" placeholder="e.g. 1" value={form.meetingId} onChange={handleChange} required />
                            </div>
                        </div>

                        <div className={styles.field}>
                            <label className={styles.label}>Description</label>
                            <textarea className={styles.textarea} name="description" placeholder="What is this poll about?" value={form.description} onChange={handleChange} rows={3} />
                        </div>

                        <div className={styles.row}>
                            <div className={styles.field}>
                                <label className={styles.label}>Majority Type</label>
                                <select className={styles.select} name="majorityType" value={form.majorityType} onChange={handleChange}>
                                    {MAJORITY_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                                </select>
                            </div>
                            <div className={styles.field}>
                                <label className={styles.label}>Electoral Body Count</label>
                                <input className={styles.input} name="electoralBodyCount" type="number" placeholder="e.g. 50" value={form.electoralBodyCount} onChange={handleChange} required />
                            </div>
                        </div>
                            ...
                            <div className={styles.candidateList}>
                                {form.candidateNames.map((name, index) => (
                                    <div key={index} className={styles.candidateRow}>
                                        <input className={styles.input} placeholder={`Candidate ${index + 1}`} value={name} onChange={(e) => handleCandidateChange(index, e.target.value)} />
                                        {form.candidateNames.length > 1 && (
                                            <button type="button" className={styles.removeBtn} onClick={() => removeCandidate(index)}>✕</button>
                                        )}
                                    </div>
                                ))}
                                <button type="button" className={styles.addBtn} onClick={addCandidate}>+ Add Candidate</button>
                            </div>
                        </div>
